...
- `[2026-07-22] doublon` : `fiche-p6-sophie-bluel.md` est resté à la racine du dépôt alors que son contenu existe déjà en deux endroits, les douze blocs de `p06/` et l'archive `_deltas/02-etape3-filtres.md`. Risque : rouvrir dans six mois une version périmée en la croyant à jour. Décision reportée, le fichier est conservé pour l'instant → racine du dossier

- `[2026-07-22] doublon` : `_template/04-bilan.md` ne contient plus qu'une note "fichier remplacé". Le gabarit réel est dans `_template/04-choix-techniques.md` et `_template/05-bilan.md`. Même décision, conservé pour l'instant → `_template/`

> Les deux sont sans risque de perte : le commit `b9909bb` les contient. Les supprimer se fait à tout moment, ils restent récupérables depuis l'historique Git.
